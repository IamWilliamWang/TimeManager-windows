...
4.4:缓存管理器改进，隐藏Cache内Sql细节，只清晰明了的显示开机时间和关机时间部分

4.4.1:增加配置文件显示和帮助。删除“导入所有数据(旧版)”功能

4.4.2:修复暗黑模式显示缺漏，关机倒计时可切换分钟或小时

4.4.3:命令行写缓存会跟随程序路径，cache支持任意文件名。缓存管理器可以取消关闭，简化内容修改判断。

4.5:缓存管理器升级内核并重构，支持自动备份，增添了显示模式菜单以及新的直观模式，修复了修改检测的问题。安装插件无需重启程序。主窗体新增显示后n条数据功能，菜单栏文字调整。睡/休眠时不打勾不会记录开机。显示配置文件独立出窗体。命令行版本更新至2.0.2。

4.5.1:修复了缓存管理的切换模式时标题混乱、点击合并后仍然提示需保存修改的问题。修复了主窗口因行数过多导致行号被遮挡的问题。

4.5.2:修复了软件定时休/睡眠情况下进程残留的问题。增加了右下角徽标和徽标的右键菜单，定时休眠和睡眠会有通知提示，也可以取消定时。
...
2.1:传入文件夹可自动补全文件名。开机记录不再支持传文件名，默认写入到程序所在文件夹。完善帮助文档