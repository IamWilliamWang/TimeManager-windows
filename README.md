...
|版本号|新功能|调整与修改|修复问题|
|:-:|:-|:-|:-|
|5.0(未来计划)|增加了使用手册，第一次打开程序会提示是否转到网页|数据管理器中不再显示也再不会更新当天/当月使用次数（测试不使用字段后会不会影响数据可视化的速度，测试删除这两个字段会不会不兼容），时长不显示小数点。数据可视化全面升级改名为数据分析器，可任意调整窗口分配比例。分析时间分布改为更换分析指标，将返回总图改为每月累计时长统计，右侧改为年月和当月累计时长两列。更换分析指标新增了每月使用次数和每日使用次数。分别是年月和当月使用次数两列，年月日和当日使用次数两列。代码大重构，将SqlExecutor合并||
|4.7.1|||修复了立刻取消里不适配暗黑模式的问题，修复了休眠倒计时可能失效的问题
|[4.7](https://github.com/IamWilliamWang/TimeManager-windows/releases/download/v4.7/TimeManager.exe)|所有窗口都支持窗口置顶，并由主界面的右键菜单内的窗口置顶进行设置。数据管理器增加删除指定范围记录功能，精准查找功能支持年月日任意组合。缓存管理器增加了修改字号功能。数据可视化图表支持数据标签的关闭和打开，支持了具体数据的气泡提示。配置管理器可以设置自动使用安全模式启动并在主界面进行适配|主界面检测后台程序时也会检测其他别名，修改弹窗文字，取消睡眠休眠如果杀后台会有ID提示，拓展功能点两次后不会再有过渡效果，透明度字样改为不透明度，不透明度后会加上%，弹窗文字、右键菜单文字和顺序变更，顶部菜单的下拉选项的字体变更。数据管理器多处文字改动，重命名函数名、注释。缓存管理器更换了默认字体字号，切换模式时会检测ScrollBar状态，右键菜单替换成“撤销、剪切、复制、粘贴、删除、全选”，直接添加ToolStripSeparator加快的开启速度。数据可视化删除了图例，删除了关闭窗口功能。配置管理器界面文字微调|主界面修复了mForm为空报错问题，修复了自动关机配置加载异常问题，修复了暗黑模式的颜色不统一问题。数据管理器修复BackupCreater不写入备份的问题，修复了InputBox设置提示后点击就会清空的问题|
|4.6.1||重命名数据管理器类名，缓存不使用默认的备份函数|解决了休/睡眠后可能导致的程序无法退出问题，修复了自动备份报错的问题
...
|4.5.4|检测没有配置文件会生成范例|将配置格式显示移入配置窗体|修正了范例格式和ConfigManager内部可能导致的所有加载错误
|4.5.3|README添加每个版本的下载链接。主界面右菜单添加隐藏右下角徽标按钮。配置文件添加了透明度设置、自动隐藏于任务栏、自动隐藏右下角徽标并同步至显示配置文件、配置文件格式。单击右下角徽标可以显示窗口|README大整理，调整结构，将更新日志改成表格更加直观。显示配置文件的说明文字修改|修复notifyIcon的残留和休眠卡死问题
|4.5.2|增加了右下角徽标和徽标的右键菜单，定时休眠和睡眠会有通知提示，也可以取消定时||修复了软件定时休/睡眠情况下进程残留的问题
|4.5.1|||修复了缓存管理的切换模式时标题混乱、点击合并后仍然提示需保存修改的问题。修复了主窗口因行数过多导致行号被遮挡的问题
...
|4.4.2|关机倒计时可切换分钟或小时||修复暗黑模式显示缺漏
|4.4.1|增加配置文件显示和帮助|废弃并删除“导入所有数据(旧版)”功能
...
|4.3.4|打开缓存管理器时立即备份，主界面关闭时自动清理备份||合并缓存与加载数据循环调用问题已解决
...
|4.2.5|添加了导入数据库向后兼容的选项|删除了原始版本的导入导出功能。新版的导入功能不会修改备份文件|修复了解压一直失败问题
|4.2.4|||修复了还原数据库一直遗留的致命错误
|4.2.3|记录开机时间、注册关机事件适配终端2.0接口并支持utf-8脚本|应用新的数据库加密算法
...
|4.1.5|为了降低不必要的操作，补丁程序所有功能成功合并至缓存管理器。补丁程序停止开发|清理缓存功能提供空文件、文件不存在提示。找不到缓存文件不再弹窗提示
...
|4.0.4|主窗口行号宽度、界面宽度自适应|隐秘功能去除。连接数据库后离线功能禁用
