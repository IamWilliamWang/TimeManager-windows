...
<br>
当前版本：4.1.0(Form版本) + 1.6.1(Console版本)
...
4.1.0:全体代码重构，过时代码全去除。注册关机事件增加失败提示。获取管理员权限功能修复，重启后不会有重复提示。数据管理任务栏提示局右。InputBox重写，更美观。休眠窗口不重要已移至Util。当大小保持原视时，数据展示自动拓宽/缩小窗体宽度。精准查找放错处理。
